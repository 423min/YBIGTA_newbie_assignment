...
def embed_passages(texts: list[str], ids: list[str], progress_callback=None) -> np.ndarray:
    ...
    if len(texts) != len(ids):
        raise ValueError("texts and ids are not same length.")
    
    PROCESSED_DIR.mkdir(parents=True, exist_ok=True)

    keys = _get_api_keys()
    if not keys:
        raise ValueError("No upstage API key found.")
...
def embed_query(query: str) -> list[float]:
    ...
    response = client.embeddings.create(model="solar-embedding-1-large-query", input=truncated_query)
