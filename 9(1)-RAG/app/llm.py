...
def generate(question: str, context: str | None = None) -> str:
    ...
    if not api_key:
        raise ValueError("Upstage API key not found.")
    ...
    response = client.chat.completions.create(model=MODEL, messages=[{"role":"user", "content":prompt}], temperature=0, max_tokens=1024)
